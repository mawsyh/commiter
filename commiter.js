...
async function main() {
  const mode = await ask("Enter the commit mode (Stochastic, Constant): ")
  ...
  if (mode == "Stochastic"){
      ...
      for (var day of weekDays){
        const ratio = parseFloat(await ask('Enter the ' + day + ' ratio to max commits (0 to 1):'));
        const appearProbability = parseFloat(await ask ("Enter the " + day + " appear probability (0 to 1): "));
        ratios[day] = ratio;
